...
public class OAuth2ResourceAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (!registeredClient.getAuthorizationGrantTypes().contains(AuthorizationGrantType.PASSWORD)) {
            ...
            //验证用户和密码
           UserDetails details =  userDetailsService.loadUserByUsername(resourceAuthentication.getUsername());
            if(null == details){
                throw new OAuth2AuthenticationException(OAuth2ErrorCodes.INVALID_GRANT);
            }
            if(!this.passwordEncoder.matches(resourceAuthentication.getPassword(), details.getPassword()))
            {
                throw new OAuth2AuthenticationException(OAuth2ErrorCodes.INVALID_GRANT);
            }

            Authentication principal = new UsernamePasswordAuthenticationToken
                    (resourceAuthentication.getUsername(),resourceAuthentication.getPassword());
